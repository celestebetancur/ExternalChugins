...
Compilation options: -a .faust2ck_tmp/Freeverb2.dsp-wrapper.cpp -lang cpp -es 1 -mcd 16 -single -ftz 0
------------------------------------------------------------ */
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a .faust2ck_tmp/Freeverb2.dsp-wrapper.cpp -lang cpp -es 1 -mcd 16 -single -ftz 0");
		m->declare("delays.lib/name", "Faust Delay Library");
		m->declare("delays.lib/version", "0.1");
		m->declare("description", "Original freeverb demo application, modified so it doesn't mix inputs to mono");
		m->declare("filename", "Freeverb2.dsp");
 ...
 * ChucK glue code
 */
static t_CKUINT stereoFreeverb_offset_data = 0;
static int g_sr = 44100;
static int g_nChans = 1;

CK_DLL_CTOR(stereoFreeverb_ctor)
{
    // return data to be used later
    stereoFreeverb *d = new stereoFreeverb;
    OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data) = (t_CKUINT)d;
    d->init(g_sr);
    d->ck_frame_in = new SAMPLE*[g_nChans];
    d->ck_frame_out = new SAMPLE*[g_nChans];
}

CK_DLL_DTOR(stereoFreeverb_dtor)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
    ...
    OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data) = 0;
}

// mono tick
CK_DLL_TICK(stereoFreeverb_tick)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
// multichannel tick
CK_DLL_TICKF(stereoFreeverb_tickf)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
CK_DLL_MFUN(stereoFreeverb_ctrl_fVslider1)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
CK_DLL_MFUN(stereoFreeverb_cget_fVslider1)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
    RETURN->v_float = (t_CKFLOAT)(d->fVslider1);
}
...
CK_DLL_MFUN(stereoFreeverb_ctrl_fVslider2)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
CK_DLL_MFUN(stereoFreeverb_cget_fVslider2)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
    RETURN->v_float = (t_CKFLOAT)(d->fVslider2);
}
...
CK_DLL_MFUN(stereoFreeverb_ctrl_fVslider3)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
CK_DLL_MFUN(stereoFreeverb_cget_fVslider3)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
    RETURN->v_float = (t_CKFLOAT)(d->fVslider3);
}
...
CK_DLL_MFUN(stereoFreeverb_ctrl_fVslider0)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
...
CK_DLL_MFUN(stereoFreeverb_cget_fVslider0)
{
    stereoFreeverb *d = (stereoFreeverb*)OBJ_MEMBER_UINT(SELF, stereoFreeverb_offset_data);
    RETURN->v_float = (t_CKFLOAT)(d->fVslider0);
}
...
CK_DLL_QUERY(stereoFreeverb_query)
{
    g_sr = QUERY->srate;

	stereoFreeverb temp; // needed to get IO channel count

    QUERY->setname(QUERY, "stereoFreeverb");
    
    QUERY->begin_class(QUERY, "stereoFreeverb", "UGen");
    
    QUERY->add_ctor(QUERY, stereoFreeverb_ctor);
    QUERY->add_dtor(QUERY, stereoFreeverb_dtor);
    
    g_nChans = max(temp.getNumInputs(), temp.getNumOutputs());
    
    if(g_nChans == 1)
        QUERY->add_ugen_func(QUERY, stereoFreeverb_tick, NULL, g_nChans, g_nChans);
    else
        QUERY->add_ugen_funcf(QUERY, stereoFreeverb_tickf, NULL, g_nChans, g_nChans);

    // add member variable
    stereoFreeverb_offset_data = QUERY->add_mvar( QUERY, "int", "@stereoFreeverb_data", FALSE );
    if( stereoFreeverb_offset_data == CK_INVALID_OFFSET ) goto error;

    
    QUERY->add_mfun( QUERY, stereoFreeverb_cget_fVslider1 , "float", "Damp" );
    
    QUERY->add_mfun( QUERY, stereoFreeverb_ctrl_fVslider1 , "float", "Damp" );
    QUERY->add_arg( QUERY, "float", "Damp" );
    

    QUERY->add_mfun( QUERY, stereoFreeverb_cget_fVslider2 , "float", "RoomSize" );
    
    QUERY->add_mfun( QUERY, stereoFreeverb_ctrl_fVslider2 , "float", "RoomSize" );
    QUERY->add_arg( QUERY, "float", "RoomSize" );
    

    QUERY->add_mfun( QUERY, stereoFreeverb_cget_fVslider3 , "float", "Stereo_Spread" );
    
    QUERY->add_mfun( QUERY, stereoFreeverb_ctrl_fVslider3 , "float", "Stereo_Spread" );
    QUERY->add_arg( QUERY, "float", "Stereo_Spread" );
    

    QUERY->add_mfun( QUERY, stereoFreeverb_cget_fVslider0 , "float", "Wet" );
    
    QUERY->add_mfun( QUERY, stereoFreeverb_ctrl_fVslider0 , "float", "Wet" );
