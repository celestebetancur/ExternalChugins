...
Compilation options: -a .faust2ck_tmp/Nlf2.dsp-wrapper.cpp -lang cpp -es 1 -mcd 16 -single -ftz 0
------------------------------------------------------------ */
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a .faust2ck_tmp/Nlf2.dsp-wrapper.cpp -lang cpp -es 1 -mcd 16 -single -ftz 0");
		m->declare("filename", "Nlf2.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("Nlf2");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = fSlow3 * (fSlow2 * fRec1[1] + fSlow1 * fRec0[1]);
			fRec1[0] = float(input0[i0]) + fSlow1 * fRec1[1] - fSlow2 * fRec0[1];
 ...
 * ChucK glue code
 */
static t_CKUINT Nlf2_offset_data = 0;
static int g_sr = 44100;
static int g_nChans = 1;

CK_DLL_CTOR(Nlf2_ctor)
{
    // return data to be used later
    Nlf2 *d = new Nlf2;
    OBJ_MEMBER_UINT(SELF, Nlf2_offset_data) = (t_CKUINT)d;
    d->init(g_sr);
    d->ck_frame_in = new SAMPLE*[g_nChans];
    d->ck_frame_out = new SAMPLE*[g_nChans];
}

CK_DLL_DTOR(Nlf2_dtor)
{
    Nlf2 *d = (Nlf2*)OBJ_MEMBER_UINT(SELF, Nlf2_offset_data);
    ...
    OBJ_MEMBER_UINT(SELF, Nlf2_offset_data) = 0;
}

// mono tick
CK_DLL_TICK(Nlf2_tick)
{
    Nlf2 *d = (Nlf2*)OBJ_MEMBER_UINT(SELF, Nlf2_offset_data);
...
CK_DLL_QUERY(Nlf2_query)
{
    g_sr = QUERY->srate;

	Nlf2 temp; // needed to get IO channel count

    QUERY->setname(QUERY, "Nlf2");
    
    QUERY->begin_class(QUERY, "Nlf2", "UGen");
    
    QUERY->add_ctor(QUERY, Nlf2_ctor);
    QUERY->add_dtor(QUERY, Nlf2_dtor);
    
    g_nChans = max(temp.getNumInputs(), temp.getNumOutputs());
    
    if(g_nChans == 1)
        QUERY->add_ugen_func(QUERY, Nlf2_tick, NULL, g_nChans, g_nChans);
    else
        QUERY->add_ugen_funcf(QUERY, Nlf2_tickf, NULL, g_nChans, g_nChans);

    // add member variable
    Nlf2_offset_data = QUERY->add_mvar( QUERY, "int", "@Nlf2_data", FALSE );
    if( Nlf2_offset_data == CK_INVALID_OFFSET ) goto error;

    
    QUERY->add_mfun( QUERY, Nlf2_cget_fEntry0 , "float", "f" );
    
    QUERY->add_mfun( QUERY, Nlf2_ctrl_fEntry0 , "float", "f" );
    QUERY->add_arg( QUERY, "float", "f" );
    

    QUERY->add_mfun( QUERY, Nlf2_cget_fEntry1 , "float", "r" );
    
    QUERY->add_mfun( QUERY, Nlf2_ctrl_fEntry1 , "float", "r" );
